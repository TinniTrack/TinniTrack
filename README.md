...
5. Complete Study No. 1 orientation:
   ...
   - finish orientation, which generates scheduled tasks through a Supabase RPC.
...
Study No. 1 requires an audiogram before task scheduling. The orientation sheet has three steps:
...
3. Finish orientation
   - The app calls `complete_study_no_1_onboarding`.
   - The backend marks the enrollment orientation complete and generates scheduled loudness-match tasks.

The dashboard stays blocked until the audiogram prerequisite is met. Once tasks have been unlocked, later HealthKit sync failures are shown as warnings instead of re-locking previously available tasks.

### Schedule Generation

The schedule is generated by the Supabase RPC `complete_study_no_1_onboarding(p_enrollment_id, p_timezone)`.
...
The dashboard fetches scheduled tasks for the enrollment, separates future scheduled tasks from completed tasks, and only allows task start when `ScheduledTask.isStartable(at:)` returns true.

### Modal Task Flow

The dashboard currently launches the loudness-match task through:
...
The full-screen modal owns its local step state and performs cleanup when dismissed. If a task has started, closing the modal requires confirmation. Cleanup stops active tone playback, route monitoring, continuity monitoring, quiet-room monitoring, volume monitoring, and aborts the active protocol run if needed.
...
The RPC verifies that the authenticated user owns the enrollment, the enrollment belongs to Study No. 1, the scheduled task is still scheduled, and the current time is within the task window. It inserts a `task_runs` row, sets protocol version `lm_v1`, merges `matched_level` into `raw_payload`, and marks the scheduled task completed.
...
- `study_enrollments`
  ...
  - Includes onboarding completion for Study No. 1 task generation.
- `scheduled_tasks`
  - Generated task schedule for an enrollment.
